...
function RecordedView(props: { config: MiraviewConfig, schedules?: Schedule[], services?: Service[] }): JSX.Element {
    if(!props.schedules || props.schedules.length === 0){
        return <Typography>録画済み番組がありません</Typography>;
    }
    // console.log(props.schedules);
    return (
        <Stack sx={{ margin: '1rem' }} spacing={2}>
          { props.schedules?.filter(schedule => schedule.state?.includes('finished'))
                                .map(record =>
